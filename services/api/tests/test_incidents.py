...
class TestAnalyzeEndpoint:
    def test_valid_csv_returns_summary(self, client, sample_csv_bytes, auth_headers):
        response = upload(client, sample_csv_bytes, auth_headers)
    ...
    def test_missing_file_field_returns_422(self, client, auth_headers):
        response = client.post("/api/incidents/analyze", headers=auth_headers)
        assert response.status_code == 422

    def test_empty_file_returns_400(self, client, empty_csv_bytes, auth_headers):
        response = upload(client, empty_csv_bytes, auth_headers)
        assert response.status_code == 400
        assert response.json()["detail"] == "El archivo está vacío."

    def test_invalid_columns_returns_400(self, client, invalid_header_csv_bytes, auth_headers):
        response = upload(client, invalid_header_csv_bytes, auth_headers)
        assert response.status_code == 400
        assert "formato incorrecto" in response.json()["detail"].lower()
        assert "customer_id" in response.json()["detail"]

    def test_invalid_records_are_counted(self, client, auth_headers):
        bad = (
        ...
        response = upload(client, bad, auth_headers)
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 2
        assert data["valid"] == 1
    ...
    def test_analyze_without_auth_returns_401(self, client, sample_csv_bytes):
        response = upload(client, sample_csv_bytes, {})
        assert response.status_code == 401


class TestExportEndpoint:
    def test_export_without_previous_analysis_returns_404(self, client, auth_headers):
    ...
    def test_export_after_analysis_returns_csv(self, client, sample_csv_bytes, auth_headers):
        analyze = upload(client, sample_csv_bytes, auth_headers)
    ...
    def test_state_is_reset_between_tests(self, client, auth_headers):
        response = client.get("/api/incidents/results/export", headers=auth_headers)
        assert response.status_code == 404
